...
public class Menu {
	 ...
	 * @return
	 * 				Which button was clicked. Options are MENU, UNDO, RESET, INFO, TUTORIAL
	 */
	...
	/**
	 * Determines which button, if any, the (x, y) coordinate falls within, assuming
	 * the game is currently showing a won screen
	 * 
	 * @param x
	 * 				x coordinate of the press
	 * @param y
	...
	public static GameEngine.ButtonPress containingButtonOfPixelWonScreen(int x, int y, boolean bonusUnlocked){
		...
		int topHeight = (int)(height - width * Constants.POPUP_WIDTH_BOUNDARY * 2);
		// Top button presses
		if (y <  topHeight && y > topHeight - height * Constants.GAME_BUTTON_HEIGHT){
			
			//Inside the bounds
			if (x > width * Constants.POPUP_WIDTH_BOUNDARY){
				
				// Width of inner screen
				int innerWidth = (int) (width * (1 - Constants.POPUP_WIDTH_BOUNDARY * 2));
				x -= width * Constants.POPUP_WIDTH_BOUNDARY;
				
				if (x < innerWidth / 3){
					return GameEngine.ButtonPress.UNDO;
				} else if (x < innerWidth * 2 / 3){
					return GameEngine.ButtonPress.MENU;
