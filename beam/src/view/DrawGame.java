...
public class DrawGame {
	...
	private void drawOutro(int width, int height, Board b) {
		...
		/*
		 * float au = GameEngine.getWonAnimationUnit(); float timeWon =
		 * GameEngine.getTimeWon(); float starBeamWidth = width / 6.0f; float
		 * squareSize = 0; int numStars = 1; if (GameEngine.getMoveCount() <=
		 * b.perfect){ numStars = 3; } else if (GameEngine.getMoveCount() <=
		 * b.par){ numStars = 2; } if(timeWon < au){ squareSize = (timeWon / au)
		 * * width; } else { squareSize = width; } float beam1X = (width - (4 *
		 * starBeamWidth)) / 2.0f; float beam1Progress = (timeWon - au) / (2 *
		 * au); float beam2Progress = 0, beam3Progress = 0; if(numStars >= 2){
		 * beam2Progress = (timeWon - (2 * au)) / (2 * au); } if(numStars == 3){
		 * beam3Progress = (timeWon - (3 * au)) / (2 * au); } float beam2X =
		 * beam1X + (1.5f * starBeamWidth); float beam3X = beam2X + (1.5f *
		 * starBeamWidth); float starheight = (height / 2.0f) + (0.2f * width);
		 * 
		 * float horBeamProgress = 0; if(timeWon >= (numStars + 2) * au){
		 * horBeamProgress = (timeWon - (numStars + 2) * au) / (2 * au); } float
		 * horBeamY = 4.5f * height / 10f; float horBeamHeight = height / 10f;
		 * 
		 * String levelEndMessage = "GOOD!"; if(numStars == 2){ levelEndMessage
		 * = "EXCELLENT!"; } if(numStars == 3){ levelEndMessage = "PERFECT!"; }
		 * 
		 * Gdx.gl.glEnable(GL10.GL_BLEND); Gdx.gl.glBlendFunc(GL10.GL_SRC_ALPHA,
		 * GL10.GL_ONE_MINUS_SRC_ALPHA); shapes.begin(ShapeType.Filled);
		 * shapes.setColor(new Color(0.4f,0.4f,0.4f,0.4f));
		 * shapes.rect(0,0,width, height); shapes.setColor(new
		 * Color(0,0,0,0.92f)); shapes.rect((width - squareSize ) / 2.0f,
		 * (height - (squareSize)) / 2.0f, squareSize, squareSize);
		 * shapes.end(); Gdx.gl.glDisable(GL10.GL_BLEND);
		 * 
		 * batch.begin(); starSprite.setSize(starBeamWidth, starBeamWidth);
		 * starSprite.setColor(Color.YELLOW); if(numStars >= 1 && timeWon >= 2 *
		 * au){ starSprite.setPosition(beam1X, starheight);
		 * starSprite.draw(batch); } if(numStars >= 2 && timeWon >= 3 * au){
		 * starSprite.setPosition(beam2X, starheight); starSprite.draw(batch); }
		 * if(numStars == 3 && timeWon >= 4 * au){
		 * starSprite.setPosition(beam3X, starheight); starSprite.draw(batch); }
		 * if(timeWon > (numStars + 3) * au){ TextBounds endTextBounds =
		 * introFont.getBounds(levelEndMessage);
		 * introFont.setColor(Color.WHITE); introFont.draw(batch,
		 * levelEndMessage, (width - endTextBounds.width) / 2.0f, (height +
		 * endTextBounds.height) / 2.0f); } batch.end();
		 * 
		 * float buttonTextProgress; if(timeWon >= (numStars + 2) * au){
		 * buttonTextProgress = (timeWon - (numStars + 2) * au) / (2 * au); }
		 * else { buttonTextProgress = 0; }
		 * 
		 * Gdx.gl.glEnable(GL10.GL_BLEND); Gdx.gl.glBlendFunc(GL10.GL_SRC_ALPHA,
		 * GL10.GL_ONE_MINUS_SRC_ALPHA); shapes.begin(ShapeType.Line);
		 * shapes.setColor(new Color(1,1,1,buttonTextProgress)); float space =
		 * width / 75.0f; float buttonWidth = Menu.wonButtonWidth * width - (2 *
		 * space); float buttonHeight = Menu.wonButtonHeight * height - (2 *
		 * space); float buttonBotPos = Menu.wonButtonBotY * height + space;
		 * shapes.rect(Menu.wonRetryButtonLeftX * width + space, buttonBotPos,
		 * buttonWidth, buttonHeight); shapes.rect(Menu.wonMenuButtonLeftX *
		 * width + space, buttonBotPos, buttonWidth, buttonHeight);
		 * shapes.rect(Menu.wonNextLevelButtonLeftX * width + space,
		 * buttonBotPos, buttonWidth, buttonHeight); shapes.end();
		 * 
		 * batch.begin(); titleFont.setColor(new
		 * Color(1,1,1,buttonTextProgress)); float centerHeight =
		 * (Menu.wonButtonBotY + Menu.wonButtonHeight / 2) * height; float
		 * halfButtonWidth = Menu.wonButtonWidth / 2 * width; String buttonText
		 * = "RETRY"; TextBounds buttonTB = titleFont.getBounds(buttonText);
		 * titleFont.draw(batch, buttonText, (width * Menu.wonRetryButtonLeftX)
		 * + halfButtonWidth - (buttonTB.width / 2.0f), centerHeight +
		 * (buttonTB.height / 2.0f)); buttonText = "MENU"; buttonTB =
		 * titleFont.getBounds(buttonText); titleFont.draw(batch, buttonText,
		 * (width * Menu.wonMenuButtonLeftX) + halfButtonWidth - (buttonTB.width
		 * / 2.0f), centerHeight + (buttonTB.height / 2.0f)); buttonText =
		 * "NEXT"; buttonTB = titleFont.getBounds(buttonText);
		 * titleFont.draw(batch, buttonText, (width *
		 * Menu.wonNextLevelButtonLeftX) + halfButtonWidth - (buttonTB.width /
		 * 2.0f), centerHeight + (buttonTB.height / 2.0f)); batch.end();
		 * Gdx.gl.glDisable(GL10.GL_BLEND);
		 * 
		 * 
		 * drawVerticalTempBeam(beam1Progress, beam1X, starBeamWidth,
		 * translateColor(GameEngine.Color.RED));
		 * drawVerticalTempBeam(beam2Progress, beam2X, starBeamWidth,
		 * translateColor(GameEngine.Color.BLUE));
		 * drawVerticalTempBeam(beam3Progress, beam3X, starBeamWidth,
		 * translateColor(GameEngine.Color.GREEN));
		 * drawHorizontalTempBeam(horBeamProgress, horBeamY, horBeamHeight,
		 * translateColor(GameEngine.Color.BLUE));
		 */
	}
	 ...
	 */
	private void drawVerticalTempBeam(float progress, float posX, float width,
			Color baseColor) {
		float trueWidth = 0;
	 ...
	 */
	private void drawHorizontalTempBeam(float progress, float posY,
			float height, Color baseColor) {
		float trueHeight = 0;
		int width = Gdx.graphics.getWidth();
		if (progress < 0 || progress > 1) {
			return;
