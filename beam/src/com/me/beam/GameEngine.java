...
public class GameEngine implements ApplicationListener {
		...
		for (Laser l : b.lasers){
			...
			if (l.getXStart() == xPos && l.getYStart() == yPos){
				...
				if (l.getXStart() < l.getXFinish()){
					...
					//New laser formed
					if (leftTile != null){
						Laser newLaser = new Laser(leftTile.getXCoord(), leftTile.getYCoord(), 
								rightTile.getXCoord(), rightTile.getYCoord(), l.getColor());
				...
				//Laser goes up
					...
					if (botTile != null){
						Laser newLaser = new Laser(botTile.getXCoord(), botTile.getYCoord(), 
								topTile.getXCoord(), topTile.getYCoord(), l.getColor());
						survivingLasers.add(newLaser);
					}
				}
		...
		for (; leftSameColor == null && xPos >= 0; xPos--){
			...
			if (possible != null){
				if (possible.getColor() == p.getColor()){
					...
					b.lasers.add(new Laser(xPos, yPos, p.getXCoord(), p.getYCoord(), p.getColor()));
				} else {
		...
		for (; rightSameColor == null && xPos < b.getNumHorizontalTiles(); xPos++){
			...
			if (possible != null){
				if (possible.getColor() == p.getColor()){
					...
					destroyed.addAll(possibleDestroy);
					b.lasers.add(new Laser(p.getXCoord(), p.getYCoord(), xPos, yPos, p.getColor()));
				} else {
					possibleDestroy.add(possible);
				}
			}
	...
	public boolean checkIfPieceDestroyed(Piece p){
		...
		for (; leftColor == Color.NONE && xPos >= 0; xPos--){
			...
			Piece atLeft = b.getPieceOnTile(b.getTileAtBoardPosition(xPos, yPos));
		...
		if (leftColor != Color.NONE && leftColor != p.getColor()){
			...
			for (; rightColor == Color.NONE && xPos < b.getNumHorizontalTiles(); xPos++){
				...
				Piece atRight = b.getPieceOnTile(b.getTileAtBoardPosition(xPos, yPos));
		...
		for (; topColor == Color.NONE && yPos >= 0; yPos--){
			...
			Piece atTop = b.getPieceOnTile(b.getTileAtBoardPosition(xPos, yPos));
		...
		if (topColor != Color.NONE && topColor != p.getColor()){
			...
			for (; botColor == Color.NONE && yPos < b.getNumVerticalTiles(); yPos++){
				...
				Piece atBot = b.getPieceOnTile(b.getTileAtBoardPosition(xPos, yPos));
